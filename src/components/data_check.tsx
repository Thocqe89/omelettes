 ...
 */
/*******  e0f87b73-e844-467b-ab89-6f39fb5e67a9  *******/
export default function DataCheck() {
    ...
    const fetchEntry = async (searchValue: string, keyToMatch: "id" | "phone") => {
        ...
        try {
            ...
            alert(t("fetch_error_message") || "An error occurred while fetching data."); // User-friendly error
            ...
            <section className="min-h-screen px-4 py-10 ">
                <div className="max-w-xl mx-auto space-y-6">
                    ...
                    <div className="flex justify-center gap-4">
                        ...
                        >
                            <RiQrScan2Line className="hover:animate-zoom-out" color="#ffffff" size={30} />
                        </Button>
                        <Button
                    ...
                    {!loading && !useCamera && (!entry || (Array.isArray(entry) && entry.length === 0)) && (
                    ...
                    {!loading && useCamera && (
                        <div className="relative bg-white dark:bg-gray-800 px-4 pt-8 pb-4 rounded-xl shadow-md overflow-hidden">
                            ...
                            >
                                <IoCloseOutline size={20} className="color-[#0d7a68]" />
                            </button>
                                ...
                                <QrReader
                                    ...
                                    onResult={(result, error) => {
                                        if (result) handleQrScan(result);
                                            ...
                                            alert(t("camera_permission_denied") || "❌ Camera permission denied.");
                                            setUseCamera(false);
                                        }
                                    }}
                                />
                            ...
                            <p className="mt-2 text-sm text-center text-gray-500">
                                {t("scanned_result") || "Scanned Result"}: {scanResult || (t("na") || "N/A")}
                            </p>
                        </div>
                    )}
                            ...
                            {entry.map((row, idx) => (
                                ...
                                >
                                    <h2 className="font-bold text-lg mb-2 text-[#0d7a68] dark:text-gray-200">
                                        {t("matched_entry") || "Matched Entry"} {entry.length > 1 ? idx + 1 : ""}
                                    </h2>
                                  <ul className="space-y-1">
  {Object.entries(row).map(([key, value]) => {
    const normalizedKeyForTranslation = key.trim().toLowerCase().replace(/\s+/g, "");
    const displayKey = t(normalizedKeyForTranslation) || key;
    const displayValue = value && String(value).trim() !== "" ? String(value) : "N/A";

    return (
      <li key={key} className="flex justify-between border-b border-green-200 pb-1 dark:border-gray-500">
